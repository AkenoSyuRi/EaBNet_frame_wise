...
class UNet_Encoder(nn.Module):
    def __init__(
        ...
        c_end = 64
        unet = []
        unet.append(nn.Sequential(GateConv2d(cin, c, k_beg, (1, 2)), NormSwitch(norm_type, "2D", c), nn.PReLU(c)))
        unet.append(nn.Sequential(GateConv2d(c, c, k1, (1, 2)), nn.PReLU(c)))
        unet.append(nn.Sequential(GateConv2d(c, c, k1, (1, 2)), nn.PReLU(c)))
        unet.append(nn.Sequential(GateConv2d(c, c, k1, (1, 2)), NormSwitch(norm_type, "2D", c), nn.PReLU(c)))
        unet.append(
            nn.Sequential(GateConv2d(c, c_end, k1, (1, 2)), NormSwitch(norm_type, "2D", c_end), nn.PReLU(c_end))
        )
...
class UNet_Decoder(nn.Module):
    def __init__(
        ...
        unet = []
        unet.append(
            nn.Sequential(GateConvTranspose2d(c_beg * 2, c, k1, (1, 2)), NormSwitch(norm_type, "2D", c), nn.PReLU(c))
        )
        unet.append(
            nn.Sequential(GateConvTranspose2d(c * 2, c, k1, (1, 2)), NormSwitch(norm_type, "2D", c), nn.PReLU(c))
        )
        unet.append(
            nn.Sequential(GateConvTranspose2d(c * 2, c, k1, (1, 2)), NormSwitch(norm_type, "2D", c), nn.PReLU(c))
        )
